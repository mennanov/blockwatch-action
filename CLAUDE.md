...
3. **Resolve version** with a three-tier fallback: GitHub Releases API (preferred — Actions runners are always authenticated to GitHub and crates.io rate-limits Actions cloud IPs) → crates.io API → a `weekly-YYYY-Wnn` sentinel.
4. **Cache** `blockwatch{,.exe}` under the step-2 directory, keyed on `runner.os` + `runner.arch` + that version string. `runner.arch` matters because the cache is shared across a repo's runners, so a mixed-architecture fleet would otherwise restore a binary for the wrong arch. The `weekly-*` sentinel exists precisely so a failed lookup still produces a key that rotates weekly instead of pinning a stale binary forever.
5. **Install cargo-binstall** via `cargo-bins/cargo-binstall`, skipped on cache hit.
6. **Install blockwatch** by calling `cargo-binstall`, *not* `cargo binstall` — the previous step ships binstall alone and no toolchain, so `cargo` may not exist. Passes `--disable-strategies compile`: without that, a target with no prebuilt binary would silently fall through to `cargo install`, which needs `cargo` and would just fail with a confusing "command not found" instead of a clear "no binary available". This step re-inspects the version for the `weekly-*` prefix and installs unpinned in that case — the sentinel is load-bearing in two steps, so keep them in sync.
